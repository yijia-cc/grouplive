...
func NewServer(timer tm.Timer, txFactory tx.TransactionFactory, userDao dao.User, jwtSigningKey string, caesarCipherOffset int) *http.ServeMux {
	serveMux := http.NewServeMux()
	router := mux.NewRouter()
	routes := getRoutes(timer, txFactory, userDao, jwtSigningKey, caesarCipherOffset)
